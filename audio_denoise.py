...
def denoise_run(name, site_input, site_output):
    try:
        rate, data = wavfile.read(site_input)  # load data

        reduced_noise = reduce_noise(y=data, sr=rate, stationary=True)  # perform noise reduction

        site_output = ('{}/{}'.format(site_output, name))  # add music.wav in site
        wavfile.write(site_output, rate, reduced_noise)  # output audio
    except Exception as denoise_ex:
        print(denoise_ex)
...
def reduce_noise(
    ...
    sr : int
      输入信号/噪声信号采样率
    y_noise : np.ndarray [shape=(# frames,) or (# channels, # frames)], real-valued
      对噪声信号进行统计计算(仅用于非平稳降噪)。
    stationary : bool, optional
      是否执行平稳降噪，或非平稳降噪，默认为False
    prop_decrease : float, optional
      减少噪声的比例(1.0 = 100%)，默认为1.0
    time_constant_s : float, optional
      时间常数，以秒为单位，计算非平稳噪声的底面
      algorithm, by default 2.0
    freq_mask_smooth_hz : int, optional
      平滑遮罩的频率范围为Hz，默认为500
    time_mask_smooth_ms : int, optional
      平滑掩码的时间范围(以毫秒为单位)，默认为50
    thresh_n_mult_nonstationary : int, optional
      仅用于非平稳降噪。，默认为1
    sigmoid_slope_nonstationary : int, optional
      仅用于非平稳降噪。，默认为10
    n_std_thresh_stationary : int, optional
      ...
      signal and noise., by default 1.5
    tmp_folder : [type], optional
      并行处理期间写入波形的临时文件夹。默认为
      default temp folder for python., by default None
    chunk_size : int, optional
      信号块的大小以减少噪声。更大的尺寸
      将占用更多的内存空间，较小的大小可能需要更长的计算时间。
      ，默认60000
      填充:int，可选
      每个信号块要填充多少。更大的垫
      需要更大的时间常数。，默认30000
    n_fft : int, optional
      填充零后加窗信号的长度。
      STFT矩阵' ' D ' '的行数为' ' (1 + n_fft/2) ' '。
      默认值' ' n_fft=2048 ' '样品，对应于一个物理
      在22050 Hz的采样速率下，即
      librosa中的默认采样率。这个值很适合用于音乐
      信号。但在语音处理中，推荐值为512，
      在22050 Hz的采样速率下对应23毫秒。
      在任何情况下，我们建议将' ' n_fft ' '设置为2的幂
      优化快速傅里叶变换(FFT)算法的速度。，默认1024      
    win_length : [type], optional
      每一帧音频的窗口都是' ' window ' ' length ' ' win_length ' '
      然后用零填充来匹配' ' n_fft ' '    
      较小的值可以提高STFT的时间分辨率
      分辨时间间隔很近的脉冲的能力)
      以频率分辨率(即辨别能力)为代价
      在频率上间隔很近的纯音)。这种效应是已知的
      由于时频局部化需要进行权衡和调整
      根据输入信号的属性' ' y ' '。
      如果未指定，默认为' ' 
      win_length = n_fft ' '。，默认为。无
    hop_length : [type], optional
      相邻STFT列之间的音频样本数。
      较小的值会增加' ' D ' '中的列数
      影响短时傅立叶变换的频率分辨率。
      如果未指定，默认为' ' win_length // 4 ' '(见下文)。，默认为。无
    use_tqdm : bool, optional
      是否显示tqdm进度条，默认为False
    n_jobs : int, optional
      要运行的并行作业数。设置为-1表示使用所有CPU内核，默认为1
      """
    if stationary:
